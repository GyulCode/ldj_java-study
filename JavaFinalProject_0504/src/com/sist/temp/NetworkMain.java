...
/*
 *   프로그램 -> 2개
 *   1) 로그인, 채팅문자열 입력 ... 입력 사용자
 *   2) 서버에서 전송되는 데이터를 출력
 *      --------------------- 쓰레드
 */

public class NetworkMain extends JFrame implements 
ActionListener, Runnable{
	...
	public NetworkMain() {
		logo=new JLabel();
		Image img=ImageChange.getImage(new ImageIcon
				("c:\\javaDev\\logo.png"),200,130);
...
//		setVisible(true);
		...
		// 로그인
		login.b1.addActionListener(this); // 여기 this는 처리메소드 actionListner를 호출한다
...
//			UIManager.setLookAndFeel("com.jtattoo.plaf.smart.SmartLookAndFeel");
	...
	public void actionPerformed(ActionEvent e) {
		...
		if(e.getSource()==b1) {
			...
			if(id.length()<1) {
				JOptionPane.showMessageDialog(this, "ID를 입력하시죠??");
				login.tf1.requestFocus();
				return;
			}
			String name=login.tf2.getText();
			if(name.length()<1) {
				JOptionPane.showMessageDialog(this, "이름을 입력하시죠??");
			...
			try {
				...
				// 서버 컴퓨터 -> ip
				// localhost 대신에는 211.238.142.공유IP
				// 읽는 위치 / 쓰는 위치
